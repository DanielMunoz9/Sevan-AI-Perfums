// Catálogo oficial completo de SEVÁN PERFUM
// Estos son TODOS los productos reales disponibles
...
// Función para generar productos con estructura completa
export function generateProductFromName(name: string, category: string): any {
  const basePrice = category === 'ARABES' ? 95000 : category === 'COLECCION' ? 120000 : 75000;
  const randomVariation = Math.floor(Math.random() * 20000) - 10000; // ±10k variation
  const price = Math.max(45000, basePrice + randomVariation);
  ...
  return {
    ...
    price: price,
    sale_price: Math.random() > 0.7 ? Math.floor(price * 0.85) : null,
    stock: 100,
...
export function getAllProducts(): any[] {
  ...
  Object.entries(PRODUCTOS_OFICIALES).forEach(([category, products]) => {
    products.forEach(productName => {
      allProducts.push(generateProductFromName(productName, category));
    });
  });
...
// Total de productos: 213
export const TOTAL_PRODUCTOS = Object.values(PRODUCTOS_OFICIALES).flat().length;